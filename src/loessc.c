...
static
void loess_workspace(int *d, int *n, double *span, int *degree, int *nonparametric, int *drop_square, int *sum_drop_sqr, int *setLf);
static
void loess_prune(int *parameter, int *a, double *xi, double *vert, double *vval);
static
void loess_grow (int *parameter, int *a, double *xi, double *vert, double *vval);
		...
		int *sum_drop_sqr, int *setLf)
{
	...
	if (nf <= 0)
		//fprintf(stderr, "span is too small\n");
		exit(mmessage(ERROR_MSG, INTERNAL_ERROR, "span is too small\n"));
	...
	if (dliv < INT_MAX)
		...
		mmessage(ERROR_MSG, INTERNAL_ERROR,
					"workspace required is too large");
	//else error("workspace required is too large");
	if (*setLf) {
		lv = lv + (D + 1) * nf * nvmax;
		liv = liv + nf * nvmax;
	}
	iv = calloc(liv, sizeof (int));
	v = calloc(lv, sizeof(double));
...
static void loess_grow(int *parameter, int *a, double *xi, double *vert,
							double *vval)
{
...
#define MSG(_m_)	fprintf(stderr, (_m_)); break; //msg = _(_m_) ; break ;
...
void F77_SUB(ehg182)(int *i)
{
	...
	//warning(msg);
	mmessage(WARNING_MSG, INTERNAL_ERROR, msg);
} /* F77_SUB */
#undef MSG
...
void F77_SUB(ehg183a)(char *s, int *nc,int *i,int *n,int *inc)
{
	...
	strcat(mess,"\n");
	//warning(mess);
	mmessage(WARNING_MSG, INTERNAL_ERROR, mess);
} /* F77_SUB */
