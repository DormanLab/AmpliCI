...
void fprint_usage(FILE *fp, const char *exe_name, const char *command, void *obj)
{
	...
	if (!command) {
		...
		fprintf(fp, "\nSYNOPSIS\n\t%s cluster [--profile FILE] [--abundance FLOAT] [--log_likelihood FLOAT] [--contaminants UINT --diagnostic FLOAT --per_candidate] [--align --scores INT INT INT] --fastq FILE --outfile FILE\n", &exe_name[start]);
		...
		fprintf(fp, "\nSYNOPSIS\n\t%s error [--partition FILE] [--abundance INT --exclude] --fastq FILE --outfile FILE\n", &exe_name[start]);
		...
		fprintf(fp, "\nSYNOPSIS\n\t%s assignment [--log_likelihood FLOAT --profile FILE] --fastq FILE --haplotype FILE --outfile FILE\n", &exe_name[start]);
		fprintf(fp, "\nDESCRIPTION\n\tAssigns reads to provided haplotypes (--haplotype) with possible lower bound on log likelihood to exclude poorly explained reads.\n");
	} else if (!strcmp(command, "daumi")){
		fprintf(fp, "\nNAME\n\t\t%s-%s -- cluster reads tagged with UMIs\n", &exe_name[start], command);
		fprintf(fp, "\nSYNOPSIS\n\t%s daumi --fastq FILE --haplotype FILE --umifile FILE --umilen UINT --outfile FILE --rho FLOAT [--profile FILE]  \n", &exe_name[start]);
		fprintf(fp, "\nDESCRIPTION\n\t Cluster reads tagged with UMIs. \n");
	}
	...
	if (!strcmp(command, "assignment") || !strcmp(command, "daumi"))
